...
class CIFAR100(Dataset):
  ...
  def download_and_process(self, train, noise_type, noise_rate, random_state):
    ...
    if train and noise_type == 'clean':
      self.images = torch.from_numpy(cifar100_train.data)
      self.labels = torch.tensor(cifar100_train.targets, dtype=torch.long)
      self.noise_rate = 0.0
      self.noise_or_not = self.labels == self.labels

      self.classes = cifar100_train.classes
      self.class_to_idx = cifar100_train.class_to_idx

    if train and noise_type != 'clean':
      self.images = torch.from_numpy(cifar100_train.data)

      clean_labels = np.array(cifar100_train.targets)
    ...
    if not train:
      self.images = torch.from_numpy(cifar100_test.data)
      self.labels = torch.tensor(cifar100_test.targets, dtype=torch.long)
      self.noise_rate = 0.0
      self.noise_or_not = self.labels == self.labels

      self.classes = cifar100_test.classes
      self.class_to_idx = cifar100_test.class_to_idx
